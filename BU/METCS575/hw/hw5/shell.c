...
/**
    @brief New print working directory command, 
    which uses library function getcwd() to implement the new internal command.
 */
void mypwd() {
    char myPwd[1024];
...
void mycd(char *arg){
    if (arg == NULL) {
        fprintf(stderr, "Missing directory argument to \"cd\"\n");
    }
    else
    {
        if (chdir(arg) != 0)
